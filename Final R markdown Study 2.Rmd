...
INCOMECLEAN$GINI <- scale(INCOMECLEAN$GINI, center = T, scale = F)
...
## model comparison
model.comparison(H1A_B_full, SJ_NULLMODEL)
model.comparison(H1A_B_full, H1A_B_Reduced2)
...
plot_h1a_b_1 <- visualize(H1A_B_Reduced,
  plot = "model",
...
plot_h1a_b_2 <- visualize(H1A_B_Reduced,
  plot = "model",
...
plot1 <- plot_grid(plot_h1a_b_1, plot_h1a_b_2)

save_plot("H1A_B_Plot.png", plot = plot1)
```
...
## reduced/removal of random effects (i.e.,slopes) for gender and age
...
## Model comparisons
model.comparison(H2A_country_full, H2A_country_reduced) 
...
H1C_H4B_DATA$polpartypower_fac <- factor(H1C_H4B_DATA$polpartypower, levels = c(
        0, 1, 2), labels = c("Right", "Center", "Left"))

## Setting reference level for pol party in power
reference_level <- "Right"
H1C_H4B_DATA$polpartypower_fac <- relevel(H1C_H4B_DATA$polpartypower_fac,
                                          ref = reference_level)
...
## Reduced removal of random effects for age and gender ot check model comparisons
...
model.comparison(H2B_Moderation_fullmodel_Country, H2B_Moderation_reducedmodel_Country)
...
plot2_country <- plot_grid(H2A_plot_Country, H2B_plot_Country, ncol = 2)

save_plot("H2A_B_Plot_country.pdf", plot = plot2_country)

## Building a table for both h2a and h2b
tab_model(H2A_country_full, H2B_Moderation_fullmodel_Country, 
          file = "H2A_B_country_Model.doc")
```
...
## Reducing model via random effects (slopes) on gender and age to check model comp 
...
## Model comparison
model.comparison(H3A_full_Country, H3A_reduced_Country)
...
H3A_plot_Country <- visualize(H3A_Country, plot = "model",
...
H3B_Moderation_full_Model_Country <- lmer(System_Justification ~ GINI*Social_Mobility +
                                             age + Gender2 +
                                     (1 | Country) + (0 + age + Gender2 | Country), 
                                     data = H1C_H4B_DATA, REML = F, 
                                     control=lmerControl(optimizer="bobyqa")) ## Converges

## Reduced model for model fit
H3B_Moderation_reduced_model_country <- lmer(System_Justification ~ GINI*Social_Mobility +
                                             age + Gender2 +
                                     (1 | Country), 
                                     data = H1C_H4B_DATA, REML = F, 
                                     control=lmerControl(optimizer="bobyqa"))
...
## model comparisons
model.comparison(H3B_Moderation_full_Model_Country, H3B_Moderation_reduced_model_country)
visualize(H3B_Moderation_full_Model_Country, plot = "residuals")
## Full model fails to converge - according to the recommendations of Meteyard & Davies, (2020) &
## Nezlek, (2008) we opt to use the simpler model that converges properly
## We dropped the random effects for age and gender (i.e., slope)



summary(H3B_Moderation_reduced_model_country)
estimates(H3B_Moderation_reduced_model_country)
tab_model(H3B_Moderation_reduced_model_country)

H3B_plot_Country <- visualize(H3B_Moderation_full_Model_Country, plot = "model",
          formula = System_Justification~Social_Mobility + Country | GINI,
          raw.data = F, sample = 42) + 
        labs(x = "Social Mobility",
             y = "System Justification")

Plot3_Country <- plot_grid(H3A_plot_Country, H3B_plot_Country)

save_plot("H3A_B_plot_Country.pdf", Plot3_Country)
## Building table for H3A and B at region level
tab_model(H3A_full_Country, H3B_Moderation_full_Model_Country, 
          file = "H3A_B_Country_Model.doc")
```
