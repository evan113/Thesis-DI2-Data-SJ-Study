...
## Models converge except for most complex preregistered model

## model comparisons
anova(H3A_NULLMODEL,H3B_base_model_Country, 
      H3B_Moderation_reduced_model_country, H3B_Moderation_full_Model_Country)

visualize(H3B_Moderation_reduced_model_country, plot = "residuals")

## According to the model comparison  the best fitting model is the most 
## complex theorized preregistered model, however due to the recommendations of 
## Meteyard & Davies, (2020) & Nezlek, (2008), we opt to select the reduced model
## per convergence issue. Residuals are also more uniform within the reduced
## model indicating better fit.
...
simple_slopes(H3B_Moderation_reduced_model_country)


H3B_plot_Country <- visualize(H3B_Moderation_reduced_model_country, plot = "model",
          formula = System_Justification~GINI + Country | Social_Mobility ,
 ...
 H3B_interaction_plot <- plot_model(H3B_Moderation_full_Model_Country, type = "pred", 
           terms = c("GINI", "Social_Mobility [-1.78, 0, 1.73]")) +
        theme_apa() +
        labs(x = "GINI",
             y = "System Justification",
             title = NULL)
...
save_plot("H3A_B_plot_Country.pdf", H3B_plot_Country)
