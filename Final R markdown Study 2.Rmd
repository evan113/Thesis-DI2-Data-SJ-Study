...
## Individual-level system justification correlations by country
```{r}
## log Income
INCOMECLEAN %>%
        group_by(Country) %>%
...
INCOMECLEAN %>%
        group_by(Country) %>%
        drop_na(LOG10USDINCOME) %>%
        do(data.frame(correlation = cor(.$System_Justification, .$income_percent, method = "pearson"),
                      pvalue = cor.test(.$System_Justification, .$income_percent)$p.value))
...
## Preliminary analyses - descriptives stats w/ anova
## Means and SD's of SJ and other variables of interest
```{r}
ddply(INCOMECLEAN, .(Country), summarise,
      "mean" = mean(System_Justification, na.rm = T), "sd" = sd(System_Justification, na.rm = T))
...
```{r, warning=FALSE}
incomecor <- subset(INCOMECLEAN, 
                    select = c(income_percent, LOGPPP, LOG10USDINCOME, 
                               Life_Satisfaction, WellinSociety, RegIncome,
...
## Individual Country specific correlations 
```{r}

## removing NA from income percent
INCOMECLEAN2 <- INCOMECLEAN2[!(is.na(INCOMECLEAN2$income_percent)), ]


ddply(INCOMECLEAN2, .(Country), summarise,
      "corr" = cor(RegIncome,
                   income_percent, use = "complete.obs",
                   method = "pearson"))

ddply(INCOMECLEAN2, .(Country), summarise,
      "corr" = cor(income_percent,
...
ddply(INCOMECLEAN2, .(Country), summarise,
      "corr" = cor(income_percent,
                   FeelingThermometer_2, use = "complete.obs",
                   method = "pearson"))


ddply(INCOMECLEAN2, .(Country), summarise,
      "corr" = cor(income_percent,
                   GlobalTrustInventory_Knowledge, use = "complete.obs",
                   method = "pearson"))

ddply(INCOMECLEAN2, .(Country), summarise,
      "corr" = cor(income_percent,
                   Meaning_of_Life, use = "complete.obs",
                   method = "pearson"))

```

## Writing data into SAV file to get country specifc correlations
```{r}
write_sav(INCOMECLEAN2, "Incomeclean2.sav")
```
