...
## Creating dataset for Sean Willins
```{r}
sean_data <- subset(dat2, select = c(
        "ResponseId","Country", "Region", "age", "Gender", "LifeSatisfatction_1", 
  "LifeSatisfatction_2", "Life_Satisfaction", "Subjectivewellbeing1", 
  "Subjectivewellbeing2", "Subjectivewellbeing3","Subjectivewellbeing4", 
  "Subjectivewellbeing5", "Subjectivewellbeing6", "Subjectivewellbeing7",
  "Subjwellbeing8", "Subjwellbeing9", "Subjewellbeing10",
  "Subjectivewellbeig11","Subjwellbeing12", "Subjwellbeing13", "Subjwellbeing14", 
  "Subjwellbeing15", "Loneliness_1", "Loneliness_2", "Loneliness_3", "Loneliness",
  "Digital_media_Usage_1", "Digital_media_Usage_2",
  "Digital_media_Usage_3", "Digital_media_Usage_4", "Digital_media_Usage_5",
  "DigitalMediause2_1", "DigitalMediause2_2", "DigitalMediause3_1", 
  "DigitalMediause3_2", "DigitalMediause3_3", "DigitalMediause3_4", "DigitalMediause3_5",
  "Hoursaday", "Hoursaday_2_TEXT", "GlobalTrustInventory_Close"
))

write_sav(sean_data, "Sean_Data.sav")
```
...
## Please NOTE THAT WE ARE  USING FORWARD STEPPING IN BUILDING all of the  
...
H1C_H4B_DATA <- H1C_H4B_DATA %>%
        drop_na(System_Justification, Gender2, age)

## Null model looking at ICC/between country differences for working data related to H1A and B
SJ_NULLMODEL <- lmer(System_Justification ~ 1 + (1 | Country),
                             data = H1A_H1B_DATA, REML = F, 
                     control = lmerControl(optimizer = "bobyqa"))

tab_model(SJ_NULLMODEL) ## 16% of variance in SJ explained by country differences
summary(SJ_NULLMODEL)
estimates(SJ_NULLMODEL) ## Design effect

ICC(SJ_NULLMODEL) 
ICCest(x = Country, y = System_Justification,
       data = H1A_H1B_DATA, alpha = 0.05, CI.type = "THD")
## 95% CI ICC - .12 to .25

## random intercept null model for working data related to H1C - H4B
SJ_NULLMODEL <- lmer(System_Justification ~ 1 + (1 | Country),
                             data = H1C_H4B_DATA, REML = F)
tab_model(SJ_NULLMODEL) ## 17% variance in SJ explained by country differences
summary(SJ_NULLMODEL)
estimates(SJ_NULLMODEL) ## Design effect
...
## Model 2 ## Preregistered and hypothesized model for H1A and H1B
...
tab_model(H1A_B_full, p.style = "numeric_stars", file = "H1A_B_model.doc")
...
H1C_Mediate_model <- lmer(Life_Satisfaction ~ WellinSociety + age + Gender2 +
...
## Checking C path - exploring total effect
...
## Full model which allows for exploration of b and c' paths
...
anova(H2B_Moderation_fullmodel_Country)
...
## Building a table for H2A and H2B
tab_model(H2A_country_full,H2B_Moderation_fullmodel_Country, 
          p.style = "numeric_stars", file = "H2AB_country_Model.doc")
```
...
h3a_marginaleffect_plot <- plot_model(H3A_full_Country, type = "pred", 
                                        terms = "GINI") +
        theme_apa() +
        labs(x = "GINI",
             y = "System Justifcation",
...
save_plot("h3a_marginal_plot.pdf", plot = h3a_marginaleffect_plot)
...
## model comparisons
anova(H3B_Moderation_full_Model_Country)
...
## Building table for H3A & B
tab_model(H3A_full_Country, H3B_Moderation_full_Model_Country,
          p.style = "numeric_stars", file = "H3AB_Country_Model.doc")

## looking at which countries are in the low medium and high social mobility 
H3A_B_data %>%
        group_by(Country, Social_Mobility_factor) %>%
        count()
```

## Hypothesis 4A - 4B at the country level
```{r}
library(interplot) ## Marginal effects plot

## Data to test h4a and h4b
H4A_B_data <- H1C_H4B_DATA %>%
        drop_na(System_Justification, age, Gender2)

## H4A & H4B - Freedom predicting SJ at Country level - introduction of random 
## slopes for our covariates age and gender (Preregistered)

H4A_B_Country_full_model <- lmer(System_Justification ~ Freedom + age + Gender2 +
                                     (1 | Country) + (0 + age + Gender2 | Country),
               data = H4A_B_data, REML = F, control=lmerControl(optimizer="bobyqa"))


## All models converge properly

## model comparisons
anova(H4A_B_Country_full_model)
visualize(H4A_B_Country_full_model, plot = "residuals")
...
summary(H4A_B_Country_full_model)
estimates(H4A_B_Country_full_model)
tab_model(H4A_B_Country_full_model)

H4A_B_Plot_Country <- visualize(H4A_B_Country_full_model, plot = "model",
...
h4a_b_marginaleffect_plot <- plot_model(H4A_B_Country_full_model, type = "pred", 
                                        terms = "Freedom") +
        theme_apa() +
        labs(x = "Freedom",
             y = "System Justifcation",
...
save_plot("h4a_b_plot.pdf", plot = H4A_B_Plot_Country)
save_plot("h4a_b_marginaleffectplot.pdf", plot = h4a_b_marginaleffect_plot)

## Building table for H4A and B at Country level
tab_model(H4A_B_Country_full_model, p.style = "numeric_stars",
          file = "H4A_B_Country_Model.doc")
```
